...
import {
  ...
  FlaskConical, UserCog, ArrowUpRight, Clock, HeartPulse,
} from "lucide-react";
...
function Dashboard() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [pat, rec, appts, doctors, invoices, rx, labs, recent, apptList, invList] = await Promise.all([
        supabase.from("patients").select("id, created_at"),
        supabase.from("medical_records").select("id", { count: "exact", head: true }),
        supabase.from("appointments").select("id, scheduled_at, status, patient_id, doctor_id"),
      ...
      // Build 30-day patient growth series
  ...
  return (
    <div className="space-y-6 max-w-7xl">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-3 backdrop-blur">
              ...
              <div>
                <div className="text-[10px] uppercase tracking-wider opacity-80">Revenue (30d)</div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="border-border/60"><CardContent className="p-5"><div className="text-xs text-muted-foreground">Revenue collected</div><div className="text-2xl font-semibold mt-1">{data ? fmtMoney(data.revenue) : "—"}</div><div className="text-[11px] text-muted-foreground mt-1">Last 30 days</div></CardContent></Card>
        <Card className="border-border/60"><CardContent className="p-5"><div className="text-xs text-muted-foreground">Outstanding</div><div className="text-2xl font-semibold mt-1 text-amber-600">{data ? fmtMoney(data.outstanding) : "—"}</div><div className="text-[11px] text-muted-foreground mt-1">Across all invoices</div></CardContent></Card>
        <Card className="border-border/60"><CardContent className="p-5 flex items-center gap-4"><Receipt className="h-8 w-8 text-primary" /><div><div className="text-xs text-muted-foreground">Invoices (30d)</div><div className="text-2xl font-semibold">{data?.invList.length ?? "—"}</div></div></CardContent></Card>
      </div>
    </div>
  );
}